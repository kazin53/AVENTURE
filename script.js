...
// Função para solicitar tela cheia de forma robusta
function requestFullscreen(element) {
    if (element.requestFullscreen) {
        element.requestFullscreen();
    } else if (element.mozRequestFullScreen) {
...
// Ativa fullscreen no primeiro toque em qualquer lugar da tela do lobby
...
// Chamamos quando a página carregar
window.addEventListener("DOMContentLoaded", enableFullscreenOnFirstTap);

// Ação do botão "Iniciar" agora só redireciona
function handleClick() {
    window.location.href = 'car.html';
}