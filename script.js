...
// NOVO: Array para guardar os dragões
let dragons = [];

// NOVO: Posições pré-definidas para os dragões
const DRAGON_SPAWN_POSITIONS = [
    { x: -70, y: 0, z: 70 },
    { x: 70, y: 0, z: 70 },
    { x: -70, y: 0, z: -70 },
    { x: 70, y: 0, z: -70 }
...
// NOVO: Constantes para o comportamento dos dragões
const DRAGON_AGGRO_RANGE = 25; // Distância para começar a perseguir
const DRAGON_ATTACK_RANGE = 2.5; // Distância para atacar
const DRAGON_SPEED = 0.1; // Velocidade de perseguição
...
function createGroundDetails() {
    // Grama espalhada (aumentado para o novo mapa)
    for (let i = 0; i < 3200; i++) {
        const grassGeometry = new THREE.ConeGeometry(0.1, 0.5, 4);
        const grassMaterial = new THREE.MeshLambertMaterial({ 
            color: new THREE.Color().setHSL(0.3 + Math.random() * 0.1, 0.8, 0.4 + Math.random() * 0.3)
        });
...
function spawnCoins() {
    // Aumenta o número de moedas para o novo mapa (3x mais moedas para o mapa 4x maior)
    const coinsToSpawn = Math.min(150 + playerLevel * 20, 600); 
...
function spawnGameTrees() {
    const numTrees = 100; // Mais árvores para uma floresta densa
...
function animate() {
    ...
    updateMachine();
    updateParticles();
    // NOVO: Chama a função que atualiza os dragões
    updateDragons();
...
function createDragon() {
    ...
    const body = new THREE.Mesh(new THREE.SphereGeometry(2, 32, 32), dragonMaterial);
    body.scale.y = 1.5;
    dragon.add(body);

    const head = new THREE.Mesh(new THREE.SphereGeometry(1.2, 32, 32), dragonMaterial);
    head.position.z = -2.5;
    dragon.add(head);

    const snout = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.6, 1.5), dragonMaterial);
    snout.position.z = -3.5;
    dragon.add(snout);

    const horns = new THREE.Group();
    const horn1 = new THREE.Mesh(new THREE.ConeGeometry(0.3, 1.5, 8), dragonMaterial);
    horn1.position.set(-1, 1.5, -2.5);
    horn1.rotation.z = -Math.PI / 4;
    horns.add(horn1);
    const horn2 = new THREE.Mesh(new THREE.ConeGeometry(0.3, 1.5, 8), dragonMaterial);
    horn2.position.set(1, 1.5, -2.5);
    horn2.rotation.z = Math.PI / 4;
    horns.add(horn2);
    dragon.add(horns);

    const tail = new THREE.Mesh(new THREE.TorusKnotGeometry(1, 0.2, 50, 10), dragonMaterial);
    tail.position.z = 2.5;
    tail.rotation.x = Math.PI / 2;
    dragon.add(tail);

    const wings = new THREE.Group();
    const wing1 = new THREE.Mesh(new THREE.PlaneGeometry(3, 4), new THREE.MeshStandardMaterial({
        color: 0xBA55D3,
        side: THREE.DoubleSide
    }));
    wing1.position.x = -2.5;
    wing1.rotation.y = -Math.PI / 2;
    wings.add(wing1);
    const wing2 = new THREE.Mesh(new THREE.PlaneGeometry(3, 4), new THREE.MeshStandardMaterial({
        color: 0xBA55D3,
        side: THREE.DoubleSide
    }));
    wing2.position.x = 2.5;
    wing2.rotation.y = Math.PI / 2;
    wings.add(wing2);
    dragon.add(wings);

    dragon.userData.wings = wings;
    dragon.userData.tail = tail;

    return dragon;
}

function spawnDragons() {
    DRAGON_SPAWN_POSITIONS.forEach(pos => {
        const dragon = createDragon();
        dragon.position.set(pos.x, 0, pos.z);
        scene.add(dragon);
        dragons.push(dragon);
    });
}
...
function updateDragons() {
    dragons.forEach(dragon => {
        ...
        dragon.position.y = 2 + Math.sin(Date.now() * 0.003) * 0.5;

        // Animação de bater asas
        if (dragon.userData.wings) {
        dragon.userData.wings.rotation.z = Math.sin(Date.now() * 0.005) * 0.5;
        }

        if (distanceToPlayer < DRAGON_AGGRO_RANGE && dragon.userData.mode === 'idle') {
        console.log('Dragão avistou o jogador!');
        dragon.userData.mode = 'chasing';
        }

        if (dragon.userData.mode === 'chasing') {
        if (distanceToPlayer > DRAGON_ATTACK_RANGE) {
            const direction = new THREE.Vector3().subVectors(player.position, dragon.position).normalize();
            dragon.position.add(direction.multiplyScalar(DRAGON_SPEED * dt * 100)); // Multiplica por 100 para ter uma velocidade visível

            dragon.lookAt(player.position);
        } else {
            // Ataque!
            dragon.userData.mode = 'attacking';
            attackPlayer(dragon);
        }
        }
    });
...
function attackPlayer(dragon) {
    if (coinsCarried > 0) {
        ...
        dragon.rotation.x = -Math.PI / 8;
        setTimeout(() => {
            dragon.rotation.x = 0;
        }, 200);
    }

    // Reseta o dragão para o modo inativo
    dragon.userData.mode = 'idle';
}
