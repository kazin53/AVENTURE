...
function init() {
    ...
    // **MODIFICAÇÕES PARA REMOVER O ZOOM E A INÉRCIA**
    controls.enableDamping = false; // Desativa a inércia, deixando a rotação "seca"
    controls.dampingFactor = 0; // Fator de suavização zero
    controls.enablePan = true;

    // Fixa a distância da câmera, impedindo o zoom
    // ALTERAÇÃO: Valor reduzido para 15 para aproximar a câmera
...
function spawnGameTrees() {
    const numTrees = 40; // OTIMIZAÇÃO: Reduzido de 100 para 30
...
function updatePlayer() {
    ...
    // Pulo
    const onGround = player.position.y <= 1.6; // AJUSTADO PARA O NOVO PERSONAGEM
    ...
    if (!onGround) {
        ...
        playerVelocity.y = 0;
        player.position.y = 1.5; // AJUSTADO PARA O NOVO PERSONAGEM
    }
    ...
    checkCollisions(); // NOVO: Chama a função de verificação de colisão
...
function updatePlayerAnimations() {
    ...
    if (isMoving) {
        player.position.y = 1.5 + Math.sin(Date.now() * 0.015) * 0.05;
        ...
        player.position.y += (1.5 - player.position.y) * 0.1;
...
function updateUI() {
    ...
    document.getElementById('nextLevelInfo').textContent = `Próximo nível: ${nextLevelReq - experience} moedas`;
...
// --- NOVO: LÓGICA DE COLISÃO AABB (Axis-Aligned Bounding Box) ---
function checkCollisions() {
    // Definir as dimensões do jogador para a colisão
    const playerSize = {
        width: 0.9,
        depth: 0.4
    };
    ...
    trees.forEach(tree => {
        const treeRadius = 1.5 * tree.scale.x; // Raio do tronco da árvore
        const distance = playerPos.distanceTo(tree.position);

        if (distance < playerSize.width / 2 + treeRadius) {
            // Vetor de direção do jogador para a árvore
            const direction = new THREE.Vector3().subVectors(playerPos, tree.position).normalize();

            // Deslocar o jogador para fora da árvore
            const separationVector = direction.multiplyScalar((playerSize.width / 2 + treeRadius) - distance);
            playerPos.add(separationVector);
        }
    });
    ...
    zombies.forEach(zombie => {
        const zombieSize = {
            width: 0.9 * zombie.scale.x,
            depth: 0.4 * zombie.scale.z
        };
        const zombiePos = zombie.position;
        const distance = playerPos.distanceTo(zombiePos);

        if (distance < playerSize.width / 2 + zombieSize.width / 2) {
            // Vetor de direção do jogador para o zumbi
            const direction = new THREE.Vector3().subVectors(playerPos, zombiePos).normalize();

            // Deslocar o jogador para fora do zumbi
            const separationVector = direction.multiplyScalar((playerSize.width / 2 + zombieSize.width / 2) - distance);
            playerPos.add(separationVector);
        }
    });
    ...
    chickens.forEach(chicken => {
        const chickenSize = {
            width: 3 * chicken.scale.x,
            depth: 2 * chicken.scale.z
        };
        const chickenPos = chicken.position;
        const distance = playerPos.distanceTo(chickenPos);

        if (distance < playerSize.width / 2 + chickenSize.width / 2) {
            // Vetor de direção do jogador para a galinha
            const direction = new THREE.Vector3().subVectors(playerPos, chickenPos).normalize();

            // Deslocar o jogador para fora da galinha
            const separationVector = direction.multiplyScalar((playerSize.width / 2 + chickenSize.width / 2) - distance);
            playerPos.add(separationVector);
        }
    });
}
...
function createZombie() {
    ...
    // Ajuste de rotação para garantir que o zumbi fique reto
    zombie.rotation.x = 0;

    // Zumbi do Roblox (o mesmo código do personagem principal)
    const skinMaterial = new THREE.MeshLambertMaterial({ color: 0x6B8E23 }); // Verde oliva
    const shirtMaterial = new THREE.MeshLambertMaterial({ color: 0x5D4A27 }); // Marrom sujo
    const pantsMaterial = new THREE.MeshLambertMaterial({ color: 0x4A4A4A }); // Cinza
    const hairMaterial = new THREE.MeshLambertMaterial({ color: 0x2B1B0E });
    const eyeMaterial = new THREE.MeshLambertMaterial({ color: 0x00FF00 }); // Olhos verdes
    const mouthMaterial = new THREE.MeshLambertMaterial({ color: 0x800000 }); // Boca vermelha
    
    // Altura da perna: 1.2. O ponto de pivô da perna é -0.6.
    // O grupo da perna é posicionado em 0.2. A base da perna está em 0.2 - 0.6 = -0.4.
    // O ponto de pivô do personagem está em y=1.5. A base está em 1.5 - 0.4 = 1.1.
    // A escala do zumbi é 1.5, então a altura da base do zumbi é 1.1 * 1.5 = 1.65.
    const yOffset = 1.65;
    
    // Escala para os zumbis
    zombie.scale.set(1.5, 1.5, 1.5); // Um pouco maior que o jogador

    const head = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.6, 0.6), skinMaterial);
    head.position.set(0, 1.7 + yOffset, 0);
    zombie.add(head);

    const hair = new THREE.Mesh(new THREE.BoxGeometry(0.65, 0.3, 0.65), hairMaterial);
    hair.position.set(0, 1.99 + yOffset, 0);
    zombie.add(hair);

    const eyeGeometry = new THREE.BoxGeometry(0.08, 0.08, 0.02);
    const leftEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    leftEye.position.set(-0.15, 1.71 + yOffset, 0.31);
    zombie.add(leftEye);

    const rightEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    rightEye.position.set(0.15, 1.71 + yOffset, 0.31);
    zombie.add(rightEye);

    const mouth = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.05, 0.02), mouthMaterial);
    mouth.position.set(0, 1.5 + yOffset, 0.31);
    zombie.add(mouth);

    const body = new THREE.Mesh(new THREE.BoxGeometry(0.9, 1.1, 0.4), shirtMaterial);
    body.position.set(0, 0.8 + yOffset, 0);
    zombie.add(body);
    ...
    leftArm.position.y = -0.5;
    leftArmGroup.add(leftArm);
    leftArmGroup.position.set(-0.625, 1.5 + yOffset, 0);
    ...
    rightArm.position.y = -0.5;
    rightArmGroup.add(rightArm);
    rightArmGroup.position.set(0.625, 1.5 + yOffset, 0);
    ...
    leftLeg.position.y = -0.6;
    leftLegGroup.add(leftLeg);
    leftLegGroup.position.set(-0.25, 0.2 + yOffset, 0);
    ...
    rightLeg.position.y = -0.6;
    rightLegGroup.add(rightLeg);
    rightLegGroup.position.set(0.25, 0.2 + yOffset, 0);
    zombie.add(rightLegGroup);

    // Salva as referências para as animações
    zombie.userData.leftArmGroup = leftArmGroup;
    zombie.userData.rightArmGroup = rightArmGroup;
    zombie.userData.leftLegGroup = leftLegGroup;
    zombie.userData.rightLegGroup = rightLegGroup;
...
function spawnZombies() {
    ZOMBIE_SPAWN_POSITIONS.forEach(pos => {
        ...
        // A posição Y agora pode ser 0, já que as partes do corpo já foram ajustadas
...
function updateZombies() {
    zombies.forEach(zombie => {
        ...
        zombie.position.y = Math.sin(Date.now() * 0.005) * 0.1; // O ajuste de base não é mais necessário aqui.
...
function attackPlayer(zombie) {
    ...
    // Reseta o zumbi para o modo inativo
    zombie.userData.mode = 'idle';
}

// --- NOVO: LÓGICA DA GALINHA ---

// Função para criar a galinha como um objeto 3D reutilizável
function createChicken() {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        crest.position.set(-0.4 + i * 0.4, 3.5, 0);
    ...
    for (let i = 0; i < 2; i++) {
        ...
        tail.position.set(-1.8 - i * 0.3, 0.5 + i * 0.2, 0);
        tail.rotation.z = Math.PI / 6;
        tail.castShadow = true;
        chicken.add(tail);
    }
    
    // Ajusta a posição vertical para que a base da galinha fique em y=0
    chicken.position.y = 2.8;

    return chicken;
}

// NOVO: Função para espalhar as galinhas no mapa
function spawnChickens() {
    const numChickens = 10; // Defina a quantidade de galinhas que você quer
    const worldRadius = worldSize * 0.85;
    ...
    for (let i = 0; i < numChickens; i++) {
        ...
        chicken.position.set(x, 0, z); // A posição y=0 já está ajustada dentro da função createChicken()
      
        // Variação de escala e rotação para um visual mais natural
        const scale = 0.5 + Math.random() * 0.5;
        chicken.scale.set(scale, scale, scale);
        chicken.rotation.y = Math.random() * Math.PI * 2;

        scene.add(chicken);
        chickens.push(chicken); // NOVO: Adiciona a galinha à lista para verificação de colisão
    }
}


// --- Captura botão voltar ---
// Garante que sempre exista um estado no histórico
window.history.pushState({ page: "game" }, "", "");

// Quando o usuário tocar em voltar, pergunta antes de sair
window.addEventListener("popstate", function () {
    const sair = confirm("Deseja realmente voltar ao lobby?");
    if (sair) {
        window.location.href = "index.html";
    } else {
        // Empurra novamente o estado para evitar sair sem querer
        window.history.pushState({ page: "game" }, "", "");
    }
});

// Se for APK (Cordova/Capacitor), também funciona
document.addEventListener("backbutton", function () {
    const sair = confirm("Deseja realmente voltar ao lobby?");
    if (sair) {
        window.location.href = "index.html";
    } else {
        // Reinsere o estado para não sair
        window.history.pushState({ page: "game" }, "", "");
    }
}, false);

// Função para encerrar o jogo de forma segura
function encerrarJogo() {
    // Para animação principal
    cancelAnimationFrame(animate);

    // Remove listeners do mouse/touch
    ...
    gameContainer.removeEventListener('mouseleave', onMouseUp);
    ...
    if (grassField) grassField.innerHTML = '';
    if (renderer && renderer.domElement) renderer.domElement.remove();

    // Opcional: limpa variáveis globais
    scene = camera = renderer = player = null;
    leftArmGroup = rightArmGroup = ambientLight = directionalLight = null;

    // Redireciona para a página de saída
    window.location.href = "about:blank"; // ou outra URL de saída
}

// Função para mostrar confirmação antes de sair
function confirmarSaida() {
    return confirm("Tem certeza que quer sair do jogo?");
}

// Intercepta o botão de voltar
window.addEventListener("popstate", function (event) {
    if (confirmarSaida()) {
        encerrarJogo();
    } else {
        // Cancela a ação de voltar
