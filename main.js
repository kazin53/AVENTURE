...
function checkCollisions() {
    // Definir o raio de colisão do jogador
    ...
    zombies.forEach(zombie => {
        const zombieRadius = 0.7 * zombie.scale.x; // Raio do zumbi
        ...
        if (distance < playerRadius + zombieRadius) {
            ...
            const overlap = (playerRadius + zombieRadius) - distance;
            playerPos.add(direction.multiplyScalar(overlap));
            playerVelocity.x = 0;
            playerVelocity.z = 0;
        }
    ...
    chickens.forEach(chicken => {
        ...
        if (distance < playerRadius + chickenRadius) {
            const direction = new THREE.Vector3().subVectors(playerPos, chicken.position).normalize();
            const overlap = (playerRadius + chickenRadius) - distance;
            playerPos.add(direction.multiplyScalar(overlap));
            playerVelocity.x = 0;
            playerVelocity.z = 0;
        }
